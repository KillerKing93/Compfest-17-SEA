...
<div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" 
     class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-8 rounded-md" role="alert">
    <p><?= esc(session()->getFlashdata('error_message')) ?></p>
</div>
<?php endif; ?>
                    ...
                    <?php if (!empty($testimonials)): ?>
                        <?php foreach ($testimonials as $index => $testimonial): ?>
                            <div x-show="activeTestimonial === <?= $index ?>" x-cloak
                                ...
                                <img src="<?= esc($testimonial['avatar'], 'attr') ?>" alt="Avatar dari <?= esc($testimonial['customer_name']) ?>" class="w-20 h-20 rounded-full mx-auto mb-4 border-2 border-teal-200 p-1 object-cover">
                                ...
                                <p class="text-gray-600 italic mb-4">"<?= esc($testimonial['review_message']) ?>"</p>
                                <!-- FIX: Menggunakan 'customer_name' sesuai dengan kolom database -->
                                <p class="font-semibold text-teal-600">- <?= esc($testimonial['customer_name']) ?></p>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="text-center text-gray-500 pt-12">Belum ada testimoni.</div>
                    <?php endif; ?>
                </div>
            </div>
